...
LAYER_TYPE_CHOICES = ChoiceEnum(["transformer", "trf_adp"])
...
class PromptedWav2Vec2Model(BaseFairseqModel):
    def __init__(self, 
                 cfg: Wav2Vec2Config, 
        ...
        self.encoder = TransformerEncoder(cfg, soft_prompt_path, n_tokens, random_range)
    ...
    def build_model(cls,
        ...
        return cls(cfg, soft_prompt_path, n_tokens, random_range, task)
    ...
    def forward(
        ...
        # ---- CNN feature extraction above this line ---- 

        # ---- Quantizer feature extraction below this line ----
        num_vars = None
        code_ppl = None
        prob_ppl = None
        curr_temp = None

        if self.input_quantizer: # False
        ...
        # ---- TransformerEncoder feature extraction below this line ----
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        # define prompt shallow
        if soft_prompt_path is not None:
            self.set_soft_prompt_embeds(soft_prompt_path=soft_prompt_path)
    ...
    def initialize_soft_prompt(
        ...
        if random_range is not None:
            init_prompt_value = torch.FloatTensor(n_tokens, 1024).uniform_(-random_range, random_range)
        else:
            init_prompt_value = self.pos_conv[0].weight[:,0,0].repeat(n_tokens, 1).detach().clone()
        self.soft_prompt = nn.Embedding(n_tokens, 1024)
    ...
    def set_soft_prompt_embeds(self, soft_prompt_path: str) -> None:
        ...
        print(f"Set soft prompt! (n_tokens: {self.n_tokens}, from: {soft_prompt_path})")

    def save_soft_prompt(self,
                         path: str,
                         filename: str = "soft_prompt.model"):
    ...
    def extract_features(
        ...
        if padding_mask is not None: # None
            x = index_put(x, padding_mask, 0)

        # concat original embedding and prompt
        # B x T x C
        learned_embed = self.soft_prompt.weight.repeat(x.size(0), 1, 1)
        x = torch.cat([learned_embed, x], dim=1)

        x_conv = self.pos_conv(x.transpose(1, 2))
        x_conv = x_conv.transpose(1, 2)
        x = x + x_conv

        if not self.layer_norm_first: #True
            x = self.layer_norm(x)
