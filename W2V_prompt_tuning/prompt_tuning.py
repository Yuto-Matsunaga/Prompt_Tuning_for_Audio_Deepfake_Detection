...
from data_utils_SSL import genSpoof_list,gen_atacklabels, Dataset_HABLA_train, Dataset_VCC_train, Dataset_ASVspoof2021_train
...
def load_model(config, device):
    ...
    model.load_state_dict(torch.load('./models/LA_model.pth', map_location=config["device"]), strict=False)
...
def save_tuned_module(config, model):
    if "input_prompt" in config["target_params"]:
        ...
        model.ssl_model.model.encoder.save_soft_prompt(os.path.join('./prompts_ckpt', config["dataset_name"]), f'{config["dataset_name"]}_{config["exp_name"]}.model')
    ...
    if "MLP" in config["target_params"]:
        ...
        torch.save(model.out_layer, os.path.join('./mlp_ckpt', config["dataset_name"], f'{config["dataset_name"]}_{config["exp_name"]}.model'))
...
def main(config):
    ...
    if config["dataset_name"] == 'HABLA':
        dev_set = Dataset_HABLA_train(config, dev_file_eval, dev_labels, config["database_dir"])
    elif config["dataset_name"] in ['VCC_ENG', 'VCC_FIN', 'VCC_MAN', 'VCC_GER']:
        dev_set = Dataset_VCC_train(config, dev_file_eval, dev_labels, config["database_dir"])
    elif config["dataset_name"] == "In_The_Wild":
        dev_set = Dataset_ITW_train(config, dev_file_eval, dev_labels, config["database_dir"])
    elif config["dataset_name"] == 'LA':
        dev_set = Dataset_ASVspoof2021_train(config, dev_file_eval, dev_labels, config["database_dir"])
    else:
        raise NotImplementedError
