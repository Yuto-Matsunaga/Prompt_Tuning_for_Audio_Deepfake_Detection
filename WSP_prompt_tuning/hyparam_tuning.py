...
import optuna
from data_utils_SSL import genSpoof_list, Dataset_HABLA_train, Dataset_VCC_train,Dataset_ASVspoof2021_eval,Dataset_ASVspoof2021_train, Dataset_ITW_train, Dataset_HABLA_eval_for_eer, Dataset_HABLA_VCC_eval,Dataset_HABLA_train_even,gen_atacklabels, Dataset_ITW_eval
...
def train(config, loss_fn, train_loader, model, optimizer, scheduler,dev_loader,val_loader, trial):
    ...
    for epoch in range(config["epochs"]):
        ...
        if (epoch+1) % 10 == 0:
            ...
            dev_eer,dev_acc = target_eer(config, model, val_loader, key_file=config["dev_meta"])
            ...
            writer.add_scalar("ACC/validation",dev_acc,epoch+1)
...
def objective(trial):
    ...
    if "MLP" in config["target_params"]:
        for n, p in model.named_parameters():
            if n == "fc2.weight" or n == "fc2.bias" or n == "fc3.weight" or n == "fc3.bias":
                p.requires_grad = True
                target_params.append(p)
    ...
    eer = train(config, loss_fn=loss_fn, train_loader=train_loader, model=model, optimizer=optimizer, scheduler=scheduler,dev_loader=dev_loader,val_loader = val_loader, trial=trial)

    return eer #EERにしよう lossに戻したかったらdev_epoch_loss
...
def main(config_):
    ...
    if config["dataset_name"] == 'HABLA':
        val_set = Dataset_HABLA_eval_for_eer(config, val_file_eval,val_labels, config["database_dir"])
    elif config["dataset_name"] in ['VCC_ENG', 'VCC_FIN', 'VCC_MAN', 'VCC_GER']:
        ...
        val_set = Dataset_ASVspoof2021_eval(val_file_eval,val_labels, config["database_dir"],n_samples=config["n_samples"])
    else:
        raise NotImplementedError

    # get parameters from optuna or config
