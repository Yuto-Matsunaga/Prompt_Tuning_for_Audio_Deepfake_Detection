...
# from prompted_wav2vec2 import PromptedWav2Vec2Model
############################
## FOR fine-tuned SSL MODEL
############################

class SSLPromptedModel(nn.Module):
    def __init__(self,
                device: str = 'cuda:0',
                soft_prompt_path: str = None,
                n_tokens: int = 5,
                random_range: float=0.5):
        super(SSLPromptedModel, self).__init__()

        cp_path = 'xlsr2_300m.pt'
        state = fairseq.checkpoint_utils.load_checkpoint_to_cpu(cp_path, None)
        # task = tasks.setup_task(state["cfg"].task)
        # task.load_state_dict(state["task_state"])
        from fairseq.dataclass.utils import merge_with_parent
        cfg = merge_with_parent(Wav2Vec2Config(), state["cfg"].model, False)
        self.model = PromptedWav2Vec2Model.build_model(cfg, soft_prompt_path, n_tokens, random_range)
        self.model.load_state_dict(state["model"], strict=False, model_cfg=state["cfg"])
        self.model.eval()
        self.device = device
        self.out_dim = 1024
...
class Model(nn.Module):
    def __init__(self, args, device):
        ...
        ####
        if with_prompt:
            self.ssl_model = SSLPromptedModel(self.device, soft_prompt_path, n_tokens, random_range)
        else:
        ...
        if mlp_path is not None:
            print(f'loading pretuned MLP from {mlp_path}')
            self.out_layer = torch.load(mlp_path)
        else:
